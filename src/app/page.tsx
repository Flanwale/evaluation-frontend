...
export default function HomePage() {
  ...
  const lastWas11Ref = useRef(false);
  

  const digits = phone.replace(/\D/g, "");
  const is11 = digits.length === 11;

  useEffect(() => {
    if (TURNSTILE_DISABLED) return;
    const now11 = digits.length === 11;

    if (now11 && !lastWas11Ref.current) {
      setTurnstileToken(null);
      turnstileRef.current?.reset();
    }
    lastWas11Ref.current = now11;
  }, [digits]);
  ...
  const handleSendCode = async () => {
    ...
    if (!digits) return toast.error("请输入手机号");
    if (!is11) return toast.error("请输入 11 位手机号");
    if (!TURNSTILE_DISABLED && !turnstileToken) return toast.error("请等待人机验证完成");

    setIsSending(true);
    try {
      const headers: Record<string, string> = { "Content-Type": "application/json" };
      if (!TURNSTILE_DISABLED) headers["x-turnstile-token"] = turnstileToken ?? "";
      ...
      if (!response.ok) {
        ...
        // 后端明确说 token 无效/过期，再 reset
        if (!TURNSTILE_DISABLED && /turnstile|人机|token|verify|expired/i.test(msg)) {
          turnstileRef.current?.reset();
          setTurnstileToken(null);
        }
    ...
    try {
      await authClient.phoneNumber.verify(
        ...
        },
        {
          ...
          // ✅ 关键：不要隐式 return toastId，必须返回 void
          onError: (c) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
          ...
          <div className="flex-1 space-y-8 animate-in slide-in-from-left-4 duration-700">
            <div className="space-y-4">
              ...
              <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight text-slate-900 leading-tight">
                新一代 <span className="text-blue-600">临床电子数据</span>
                <br />
                采集管理系统
              </h1>
                ...
                <CardContent className="pt-10 pb-10 text-center space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-slate-500">
                      当前用户: {session.user.name || session.user.phoneNumber}
                    </p>
                  </div>
                ...
                <CardContent className="space-y-4 pt-4">
                  ...
                  <div className="flex gap-3">
                    ...
                    <Button
                      ...
                      disabled={isSending || (!TURNSTILE_DISABLED && !turnstileToken) || !is11}
                    >
                      {isSending ? (
                  ...
                  <Button
                    className="w-full h-11 bg-slate-900 hover:bg-slate-800 text-lg mt-2"
                    onClick={handleLogin}
      ...
      <footer className="py-6 text-center text-xs text-slate-400 border-t bg-white">
        <p>© 2026 Clinical Data Center. All rights reserved. | 隐私政策 | 服务条款 cicd test</p>
      </footer>
    </div>
  );
}
